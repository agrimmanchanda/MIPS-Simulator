...
Memory::Memory (std::ifstream &binary) {
  ...
  ADDR_DATA.assign(0x4000000, 0); //bytes
  ...
  // std::copy(&bin_array[0], &bin_array[bin_size], back_inserter(ADDR_INSTR));

  //copies into correct 32-bit vector
  for (unsigned int i = 0; (i < sizeof(bin_array)); i++) {
    // int ar_i = 4*i;
    //
    // ADDR_INSTR[i] = ((u8)bin_array[ar_i] << 24) | ((u8)bin_array[ar_i+1] << 16) | ((u8)bin_array[ar_i+2] << 8) | (u8)bin_array[ar_i+3];
...
mips_error Memory::data_read_hw (uint32_t address, int16_t &data) {
  ...
  else if ((address & 0x1) != 0) {            //checks for misalign
    return -11;
  }
...
mips_error Memory::data_write_w (uint32_t address, int32_t data) {
  ...
  else if ((address >= 0x24000000) || (address < 0x20000000) || ((address & 0x3) != 0)) {  //checks for out of memory range error
    return -11;
  }
  else {
    //remove offset
...
mips_error Memory::data_write_hw (uint32_t address, int16_t data) {
  ...
  else if ((address >= 0x24000000) || (address < 0x20000000) || ((address & 0x1) != 0)) {  //checks for out of memory range error
    return -11;
  }
  else {
    uint32_t data_i = address - 0x20000000;
